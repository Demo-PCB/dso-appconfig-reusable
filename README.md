# Azure App Configuration Reusable Workflows
...
dev:
  appconfig-name: myapp-appconfig-dev
  resource-group: rg-myapp-dev

uat:
  appconfig-name: myapp-appconfig-uat
  resource-group: rg-myapp-uat

prd:
  appconfig-name: myapp-appconfig-prd
...
### CD Workflows: `_cd-appconfig-{env}.yml`
...
#### `_appconfig-rollback.yml` (DEV/UAT)
...
#### `_appconfig-rollback-prd.yml` (PRD)
...
```bash
az ad app create --display-name "GitHub-AppConfig-OIDC"
```
...
az role assignment create \
  ...
  --scope /subscriptions/<sub-id>/resourceGroups/<rg>/providers/Microsoft.AppConfiguration/configurationStores/<name>
```
...
**"No appconfig-name found for environment"**
- Verify `config.yml` has the environment section with `appconfig-name` and `resource-group`
